...
public class ZipUtil {
	...
	protected static Log logger = LogFactory.getLog(ZipUtil.class);
	/**
	 * 递归压缩文件夹
	 * 
	 * @param srcRootDir
	...
	private static void zip(String srcRootDir, File file, ZipOutputStream zos)
		...
		if (file.isFile()) {
			...
			if (index != -1) {
				int len = srcRootDir.length();
				if(!srcRootDir.endsWith(File.separator)){
					len = srcRootDir.length() + File.separator.length();
				}
				subPath = subPath.substring(len);
			}
	...
	public static void zip(String srcPath, String zipPath, String zipFileName)
			throws Exception {
		if (!StringUtils.isNullOrEmpty(srcPath) && !StringUtils.isNullOrEmpty(zipPath)
				&&!StringUtils.isNullOrEmpty(zipFileName)) {
			...
			try {
				...
				if (srcFile.isDirectory() && zipPath.indexOf(srcPath) != -1) {
					logger.error("压缩文件保存的路径不能为源文件路径的子文件夹");
				}else{
...
//						zipFile.delete();
	...
	public static void unzip(String zipFilePath, String unzipFilePath,
		...
		if (!StringUtils.isNullOrEmpty(zipFilePath)
				&&!StringUtils.isNullOrEmpty(unzipFilePath)) {
			...
			while (entries.hasMoreElements()) {
				...
				entryFilePath = unzipFilePath + File.separator
						+ entry.getName();
